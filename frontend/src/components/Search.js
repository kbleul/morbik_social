...
const getSearchSuggestions = async(word) => {
  ...
  return (
    <div className="w-full flex items-center justify-center relative">
        <input onChange={e => {setsearch(e.target.value); getSearchSuggestions(e.target.value)}} type="search" value={search} placeholder="Search..." className="w-10/12 mr-1 px-8 py-2 bg-slate-600 border-2 border-slate-400 rounded-full"/>
        ...
        {suggestions.length > 0 && 
          <ul className="absolute top-12 w-2/4 bg-gray-200 text-black max-h-[27vh] overflow-y-hidden hover:overflow-y-scroll">
