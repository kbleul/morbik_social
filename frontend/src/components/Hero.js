  ...
  return (
    <section className="bg-rose-300">
        ...
        <div className="flex absolute top-[70%]  w-2/5 ml-[30%] text-white  bg-[rgba(0,0,0,.5)]">
         <img className="w-64 h-64 rounded-[15rem] p-2" src={ user.profilePicture === "" ? avatar : `/public/data/uploads/${user.profilePicture}`} alt="profile" />
         <div>
            <h4 className="ml-[5%] mt-[15%] font-extrabold text-xl">{user.username}</h4>
            <p className="ml-[5%] px-[ 2% ] pt-[5%] pb-[10%] h-[40%]">{user.disc === "" ? "..." : user.disc}</p>
